...
std::string LangManyToMany = R"sql(
    "ID"    INT PRIMARY KEY,
    "LANG"  REFERENCES LANG(ID) ON DELETE CASCADE,
)sql";
...
std::string CREATE_COLLUMNS = R"sql(
    ...
    CREATE TABLE IF NOT EXISTS KWD_CONT(
        "ID"      INT PRIMARY KEY,
        "LANG_ID"  REFERENCES LANG(ID) ON DELETE CASCADE, 
        "COLOR0"   INT NOT NULL,
        "COLOR1"   INT NOT NULL,
        "STARTING_KEYWORD" TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS KWD_CONT_STRING(
        "ID" INT PRIMARY KEY,
        "KWD_CONT_ID" REFERENCES KWD_CONT(ID) ON DELETE CASCADE,
        "TEXT" TEXT NOT NULL,
...
std::string ManyToMany = R"sql(
    ...
    CREATE TABLE IF NOT EXISTS LANG_KWD_CONT(
      %MANY_TO_MANY
      "KWD_CONT"   REFERENCES KWD_CONT( ID ) ON DELETE CASCADE,
